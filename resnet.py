...
# 데이터셋 로드 및 클래스 목록 확인
train_dataset = ImageFolder(root=r'C:\Users\edu31\YoloV8\dataset_cnn\train')
...
def predict_images_in_folder(folder_path):
    for file_name in os.listdir(folder_path):
        if file_name.endswith('.jpg') or file_name.endswith('.png'):
            ...
            print(f"예측 중: {image_path}")
            try:
                predicted_label, class_name = predict_image(image_path)
                print(f"예측 결과: {file_name} -> 클래스 {predicted_label}, 클래스 이름: {class_name}")
            except Exception as e:
                print(f"예측 실패 ({file_name}): {e}")


# 폴더 경로 설정 및 실행
folder_path = r'C:\Users\edu31\YoloV8\camera_snapshots'
